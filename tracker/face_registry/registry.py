...
class FaceRegistry:
    ...
    high-quality face detections. New faces are matched against galleries
    to assign or recover employee identities.
    """
    ...
    def add_features(self, employee_id: str, features: np.ndarray, frame_id: int = 0) -> int:
        """
        Add face features to an employee's gallery with deduplication.

        Args:
            employee_id: Employee identifier
            features: Array of shape (N, 128) or (128,)
    ...
    def identify(self, feature: np.ndarray, exclude_ids: Optional[set] = None, current_frame: int = 0) -> Tuple[Optional[str], float, int]:
        """
        Match a face feature against all employee galleries with time weighting.
    ...
    def identify_best_match(self, face_buffer: List[Tuple[np.ndarray, int, float]], exclude_ids: Optional[set] = None, current_frame: int = 0) -> Tuple[Optional[str], float, int, Optional[np.ndarray]]:
        """
        Match multiple face features against all employee galleries.
        Returns the best match across all buffered features.

        Args:
            face_buffer: List of (feature, frame_id, quality_score) tuples
            exclude_ids: Optional set of employee_ids to skip
            current_frame: Current frame number for time weighting

        Returns:
            Tuple of (employee_id, best_distance, gallery_feature_idx, best_face_feature)
            or (None, inf, -1, None) if no match
        """
        if not face_buffer:
        ...
        for face_feat, frame_id, quality_score in face_buffer:
            emp_id, dist, idx = self.identify(face_feat, exclude_ids, current_frame)
            if dist < best_dist:
                best_dist = dist
                best_id = emp_id
                best_idx = idx
        ...
        return best_id, best_dist, best_idx, best_face_feat
